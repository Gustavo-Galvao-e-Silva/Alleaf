...
def research_node(state: TherapySessionState):
    ...
    query_topics = [
        ...
        "work stress and social anxiety",
        "sleep quality",
    ]
    all_evidence = []
    ...
    evidence_context = (
        "\n".join([f"- {text}" for text in unique_evidence])
        if unique_evidence
        else "No recent logs."
    )

    safe_notes = (
        user_notes if user_notes.strip() else "The user is starting a general check-in."
    )
    safe_evidence = (
        evidence_context
        if evidence_context.strip()
        else "No previous history available."
    )
    ...
    try:
        ...
        agenda_response = llm.invoke(
            [
                SystemMessage(content=get_researcher_system_prompt()),
                HumanMessage(
                    content=agenda_prompt
                ),  # Google GenAI prefers a HumanMessage following a SystemMessage
            ]
        )
    ...
    try:
        opening_res = llm.invoke(
            [
                SystemMessage(
                    content=get_therapist_system_prompt(
                        user_id, unique_evidence, agenda
                    )
                ),
                HumanMessage(
                    content=f"Based on the given agenda and user information, give 1-2 warm, guided welcome messages to kick-off the therapy."
                ),
            ]
        )
...
def therapist_node(state: TherapySessionState):
    ...
    system_prompt = get_therapist_system_prompt(user_id, evidence_str, agenda)
    ...
    if response.tool_calls:
        for tool_call in response.tool_calls:
            ...
            print(
                f"--- AGENT TOOL CALL: Searching for '{query}' for user '{user_id}' ---"
            )
            search_result = search_user_history.invoke(
...
def wrap_up_node(state: TherapySessionState):
    ...
    exercise_prompt = get_exercises_system_prompt(agenda)
    summary_prompt = get_summary_system_prompt(agenda)
    history = "\n".join(
        [
            f"{'User' if isinstance(m, HumanMessage) else 'Therapist'}: {m.content}"
    ...
    try:
        summary_res = llm.invoke(
            [SystemMessage(content=summary_prompt), HumanMessage(content=history)]
        )
